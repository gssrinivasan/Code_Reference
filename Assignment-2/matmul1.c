...
int main(int argc, char *argv[]) {
    ...
    double sequential, thread_parallel, openmp_parallel;
    if (argc < 2) {
    ...
    sequential=elapsed_base * 1.0e3;
    thread_parallel=((elapsed_base*1.0e3) /(num_tasks*(elapsed_pthread*1.0e3)));
    openmp_parallel=((elapsed_base*1.0e3) /(num_tasks*(elapsed_openmp*1.0e3)));

    printf("======================================================================================================\n");
    printf("Matrix Multiplication: A[M][K] * B[k][N] = C[M][N], M=K=N=%d, %d threads/tasks\n", N, num_tasks);
    printf("------------------------------------------------------------------------------------------------------\n");
    printf("Performance:\t\tRuntime (ms)\t Speed up(ms)\t MFLOPS \t\tError (compared to base)\n");
    printf("------------------------------------------------------------------------------------------------------\n");
    printf("matmul_base:\t\t%4f\tNOT APPLICABLE\t%4f \t\t%g\n",elapsed_base*1.0e3,((((2.0*N)*N)*N)/(1.0e6*elapsed_base)),maxerror(N,N,C_base,C_base));
    printf("matmul_pthread:\t\t%4f\t%4f\t%4f \t\t%g\n", elapsed_pthread*1.0e3,thread_parallel,((((2.0*N)*N)*N)/(1.0e6*elapsed_pthread)),maxerror(N,N,C_base,C_pthread));
    printf("matmul_openmp:\t\t%4f\t%4f\t%4f \t\t%g\n", elapsed_openmp*1.0e3,openmp_parallel,((((2.0*N)*N)*N)/(1.0e6*elapsed_openmp)),maxerror(N,N,C_base,C_openmp));
    free(heap_buffer);
    return 0;
}
...
/*{ 
   int i, j, k;
   int chunk = 10;
    #pragma omp parallel shared(N,M,K,A,B) private(i,j,k) num_threads(num_tasks)
    {
...
int i,j,k;
REAL temp;
#pragma omp parallel for shared(A,B,C,M,K,N) private(j,k) num_threads(num_tasks) schedule (dynamic, 10)
     ...
     for(j=0;j<N;++j)
        {
         ...
         {
         temp=0.0;
         #pragma omp parallel for reduction(+:temp) private(k) shared(A,B,C,i,j)
         for(k=0;k<K;++k)
            {
             temp = A[i][k] * B[k][j];
            }   
