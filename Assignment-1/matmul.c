...
void matmul_row1D (int M, int K, int N, REAL A[][K], REAL B[][N], REAL C[][N], int num_tasks)
{
	// consider 1 col from B and A matrix to generate 1 row in C matrix
        ...
        int num_of_matdata_left = N % num_tasks; // calculated for N not being divisible by #tasks
...
if (N%no_of_rows_in_block !=0)
   {
    for (residue=0;residue<N;)//forout
        {
         /*
          the first "if" for the completed row condition, is to check when we need to
          move to the lower half of the C matrix computation. Completed_rows variable 
          gives the N value. Once the N elements are completed, we need to move to the 
          next block computation, which will follow the row below the already computed 
          one.
         */
         if (residue != completed_rows )//ifmain
            {
        	 ...
        	 {
                 /*
                  The "if" here for the "no_of_blocks" is used for the computation 
                  of the left-over block data to be considered,
                   eg., for ./matmul 32 5
